...
func Test_hintCalculator_GetHints(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			mustCreateNewGameForTest(GameModeTemplate, &ChallengeGames[0]).board,
			[]Hint{
				{
					Value:  18,
					Method: EvalMethodSum,
			...
			if tt.want != nil {
				...
				if !reflect.DeepEqual(gotHints, wantMap) {
					for _, hint := range gotHints {
						t.Logf("hint %s %v %d, %s", PrintEvalMethod(hint.Method), hint.Path, hint.Value, tt.board.PrintBoard(func(c CellValuer, index int, padded string) string {
